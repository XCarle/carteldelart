(function(){
    ...
    ** INITIALISATION
    **/
    // on récupère les dernières données d'opinions
    $http({method: 'GET', url:'/lastopinion.json?id_visitor='+$scope.id_visitor})
    .success(function(data){
        if (data.length > 0){
            $scope.lastOpinion = data;
            console.log("last opinion : ", data);
            
            // on teste si on doit redemander l'authentification : 
            var d = new Date();
            if (new Date($scope.lastOpinion[0].lastopinion.creation_date) > new Date(d.setHours(d.getHours() - 1))){
                $scope.showAuthModal = false;
                $scope.latitude = $scope.lastOpinion[0].lastopinion.latitude;
                $scope.longitude = $scope.lastOpinion[0].lastopinion.longitude;
                $scope.getCreationId();
            } else {
    ...
    **/
    ...
    $scope.getCreationDetail = function(id){
        ...
        $http({method: 'GET', url:'/creationsWithId.json?id_creation='+id})
        .success(function(data){
            $scope.creation = data;
            console.log("parcours.ids : ", $scope.creation);
            
            // on récupère le nombre de vues
            $http({method: 'GET', url:'/nbViewWithId.json?id_creation='+id})
            .success(function(dataView){
                console.log("nbViewWithId : ", dataView);
                if (dataView && dataView[0]){
                    console.log("nb view : ", dataView[0].creationdetails.count);
                    
                    // on ajoute une vue
                    $http({method: 'PUT', url:'/incrcreationview.json?id_creation='+id})
                    .success(function(data){
                        
                        $http({method: 'GET', url:'/incrcreationview.json?id_creation='+id})
    ...
    $scope.nextCreation = function(){
        console.log("next creation");
        
        $scope.collected = true;
        $scope.registered = true;
        // on écrit l'avis pour l'utilisateur
        ...
        .success(function(data){
            console.log("opinion return data : ", data);
        
            // récupérer la prochaine creation
            $scope.getCreationId();
        })
        .error(function(data, status, header, config){
                console.log("error");
        ...
        // on demande la nouvelle oeuvre à afficher
        //$scope.getCreationDetail(parcours.id)
    }
    
    $scope.selectCurrentPosition = function(){
        console.log("select current position");
        $scope.showSpinner = !$scope.showSpinner;
        $scope.showLocationConfig = false;
        
         $scope.$watchGroup(["latitude", "longitude"], function(newValue, OldValue, scope){
            console.log("selectCurrentPosition : ", newValue[0]);
            if (newValue[0] > 0 && newValue[1] > 0){
                console.log("selectCurrentPosition > 0 )");
                $scope.getCreationId();
            }
        });
